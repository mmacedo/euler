...
for j in 0..19
  for i in 0..16
    number = grid[i,4].collect{|v| v[j] }.inject :*
    largest = number if number > largest
  end
end

# Find largest diagonal left to right
# d - distance from top left corner
# d2 - diagonal distance from top left end
for d in 0..16
  for d2 in 0..(16-d)
    # Starting from the top side (d is columns)
    number = grid[d2,4].each_with_index.collect{|v,i| v[d + i + d2] }.inject :*
    largest = number if number > largest

    # Starting from the left side (d is rows)
    number = grid[d + d2,4].each_with_index.collect{|v,i| v[i + d2] }.inject :*
    largest = number if number > largest
  end
end

# Find largest diagonal right to left
# d - distance from top right corner
# d2 - diagonal distance from top right end
for d in 0..16
  for d2 in 0..(16-d)
    # Starting from the top side (d is columns)
    number = grid[d2,4].each_with_index.collect{|v,i| v[19 - i - d2 - d] }.inject :*
    largest = number if number > largest

    # Starting from the right side (d is rows)
    number = grid[d + d2,4].each_with_index.collect{|v,i| v[19 - i - d2] }.inject :*
    largest = number if number > largest
  end
end
